...
export const QuickOpenFileRow = memo(function QuickOpenFileRow({ path, fileName, dirname, folderLabel, matchIndices, locationSuffix, onSelect }: QuickOpenFileRowProps) {
  ...
  return (
    ...
    >
      ...
      <span className="flex min-w-0 flex-1 items-center gap-1 overflow-hidden">
        ...
        {displayDir ? <span className="truncate text-[13px] leading-[22px] text-[var(--hb-quick-open-path)]">{displayDir}</span> : null}
        {locationSuffix ? <span className="shrink-0 text-[13px] leading-[22px] text-[var(--hb-quick-open-path)]">{locationSuffix}</span> : null}
        {folderLabel ? <span className="ml-auto shrink-0 truncate rounded-full bg-muted px-1.5 text-[11px] leading-[18px] text-muted-foreground">{folderLabel}</span> : null}
      </span>
    </CommandItem>
  )
})
