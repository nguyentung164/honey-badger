'use client'

import { useSortable } from '@dnd-kit/sortable'
import { CSS } from '@dnd-kit/utilities'
import { GitCompare, X } from 'lucide-react'
import { memo, useCallback, type CSSProperties } from 'react'
import { GitFileStatusBadge, type GitFileStatusCode } from '@/components/git/GitFileStatusBadge'
import { MaterialFileIcon } from '@/components/icons/MaterialFileIcon'
import { cn } from '@/lib/utils'
import { EditorTabContextMenu, type EditorTabMenuActions } from '@/pages/editor/editor-area/EditorTabContextMenu'
import { EXPLORER_GIT_LABEL_CLASS } from '@/pages/editor/explorer/explorerGitDecorations'
import type { EditorTabSummary } from '@/pages/editor/hooks/useEditorTabSelectors'

export type EditorTabItemProps = {
  tab: EditorTabSummary
  tabIndex: number
  tabCount: number
  active: boolean
  gitStatus: GitFileStatusCode | null
  showStickySeparator?: boolean
  /** Stable function — actions are built lazily when the context menu opens. */
  getTabMenuActions?: (tab: EditorTabSummary, tabIndex: number) => EditorTabMenuActions
  onSelectTab: (tabId: string) => void
  onCloseTab: (tabId: string) => void
  onPinTab?: (tabId: string) => void
  setTabRef: (tabId: string, el: HTMLDivElement | null) => void
}

type EditorTabItemBodyProps = EditorTabItemProps & {
  tabRef: (el: HTMLDivElement | null) => void
  tabStyle?: CSSProperties
  dragProps?: {
    attributes: Record<string, unknown>
    listeners: Record<string, unknown>
  }
  isDragging?: boolean
}

const EditorTabItemBody = memo(function EditorTabItemBody({
  tab,
  tabIndex,
  tabCount,
  active,
  gitStatus,
  showStickySeparator,
  getTabMenuActions,
  onSelectTab,
  onCloseTab,
  onPinTab,
  tabRef,
  tabStyle,
  dragProps,
  isDragging = false,
}: EditorTabItemBodyProps) {
  const isPreview = tab.isPreview && !tab.isPinned

  const labelButton = (
    <button
      type="button"
      className="flex h-full min-w-0 flex-1 items-center gap-1.5"
      onClick={() => onSelectTab(tab.id)}
      onMouseDown={e => {
        if (e.button === 1) {
          e.preventDefault()
          onCloseTab(tab.id)
        }
      }}
      onDoubleClick={() => onPinTab?.(tab.id)}
    >
      {tab.isCompare ? (
        <GitCompare className="h-3.5 w-3.5 shrink-0 opacity-80" aria-hidden />
      ) : (
        <MaterialFileIcon name={tab.relativePath} size={14} className="h-4.5 w-4.5 shrink-0 opacity-90 relative top-[-0.5px]" />
      )}
      <span className={cn('min-w-0 truncate text-[13px] h-[20px]', (tab.isDirty || isPreview) && 'italic', gitStatus ? EXPLORER_GIT_LABEL_CLASS[gitStatus] : null)}>
        {tab.tabLabel}
      </span>
      {gitStatus ? <GitFileStatusBadge status={gitStatus} variant="trailing" size="sm" /> : null}
    </button>
  )

  const closeButton = (
    <div className="relative flex h-5 w-5 shrink-0 items-center justify-center">
      {tab.isDirty ? (
        <span className="pointer-events-none absolute inset-0 flex items-center justify-center group-hover:opacity-0" aria-hidden>
          <span className="size-2.5 rounded-full bg-current" />
        </span>
      ) : null}
      <button
        type="button"
        className="flex h-full w-full items-center justify-center rounded p-0.5 opacity-0 hover:bg-muted group-hover:opacity-100"
        aria-label="Close"
        onPointerDown={e => e.stopPropagation()}
        onMouseDown={e => e.stopPropagation()}
        onClick={e => {
          e.stopPropagation()
          onCloseTab(tab.id)
        }}
      >
        <X className="h-4 w-4" />
      </button>
    </div>
  )

  const tabChromeClassName = cn(
    'group flex h-full max-w-[280px] shrink-0 items-center gap-1 border-r border-t-2 px-2 text-xs',
    active ? 'border-t-[#0078d4] bg-background text-foreground' : 'border-t-transparent bg-muted/10 text-muted-foreground hover:bg-muted/40',
    isPreview && !active && 'opacity-80',
    tab.isSticky && 'bg-muted/25',
    isDragging && 'bg-background opacity-95 shadow-md ring-1 ring-primary/30'
  )

  return (
    <div
      ref={tabRef}
      style={tabStyle}
      className={cn(
        'flex h-full shrink-0 items-stretch',
        dragProps && 'cursor-grab touch-none active:cursor-grabbing',
        isDragging && 'z-30 cursor-grabbing'
      )}
      {...(dragProps?.attributes ?? {})}
      {...(dragProps?.listeners ?? {})}
    >
      {showStickySeparator ? <div className="mx-0.5 w-px shrink-0 self-stretch bg-border/80" aria-hidden /> : null}
      {getTabMenuActions ? (
        <EditorTabContextMenu tab={tab} tabIndex={tabIndex} tabCount={tabCount} onSelectTab={onSelectTab} getActions={() => getTabMenuActions(tab, tabIndex)}>
          <div className={tabChromeClassName}>
            {labelButton}
            {closeButton}
          </div>
        </EditorTabContextMenu>
      ) : (
        <div className={tabChromeClassName}>
          {labelButton}
          {closeButton}
        </div>
      )}
    </div>
  )
})

export const EditorSortableTabItem = memo(function EditorSortableTabItem({ tab, setTabRef, ...rest }: EditorTabItemProps) {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: tab.id })

  const mergeTabRef = useCallback(
    (el: HTMLDivElement | null) => {
      setNodeRef(el)
      setTabRef(tab.id, el)
    },
    [setNodeRef, setTabRef, tab.id]
  )

  return (
    <EditorTabItemBody
      tab={tab}
      setTabRef={setTabRef}
      {...rest}
      tabRef={mergeTabRef}
      tabStyle={{
        transform: CSS.Transform.toString(transform),
        transition,
      }}
      dragProps={{ attributes, listeners }}
      isDragging={isDragging}
    />
  )
})

export const EditorTabItem = memo(function EditorTabItem({ tab, setTabRef, ...rest }: EditorTabItemProps) {
  const tabRef = useCallback(
    (el: HTMLDivElement | null) => {
      setTabRef(tab.id, el)
    },
    [setTabRef, tab.id]
  )

  return <EditorTabItemBody tab={tab} setTabRef={setTabRef} {...rest} tabRef={tabRef} />
})
