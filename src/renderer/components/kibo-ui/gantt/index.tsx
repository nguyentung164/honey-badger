...
export type GanttUiLabels = {
  sidebarTaskColumn: string
...
const buildYearBlock = (year: number): TimelineData[number] => ({
  year,
  quarters: new Array(4).fill(null).map((_, quarterIndex) => ({
    months: new Array(3).fill(null).map((_, monthIndex) => {
      const month = quarterIndex * 3 + monthIndex
      return { days: getDaysInMonth(new Date(year, month, 1)) }
    }),
  })),
})

const buildTimelineFromYearRange = (minYear: number, maxYear: number): TimelineData => {
  const low = Math.min(minYear, maxYear)
  const high = Math.max(minYear, maxYear)
  const out: TimelineData = []
  for (let y = low; y <= high; y++) {
    out.push(buildYearBlock(y))
  }
  return out.length > 0 ? out : createInitialTimelineData(new Date())
}
...
const defaultGanttLabels: GanttUiLabels = {
  sidebarTaskColumn: 'Issues',
...
const WEEK_LETTERS_GANTT = ['S', 'M', 'T', 'W', 'T', 'F', 'S'] as const

function groupMonthDayIndicesByWeek(daysCount: number, y: number, monthIndex: number): { start: number; len: number }[] {
  const groups: { start: number; len: number }[] = []
  let segmentStart = 0
  for (let i = 1; i < daysCount; i++) {
    const prev = new Date(y, monthIndex, i)
    const cur = new Date(y, monthIndex, i + 1)
    if (!isSameWeek(prev, cur, { weekStartsOn: 0 })) {
      groups.push({ start: segmentStart, len: i - segmentStart })
      segmentStart = i
    }
  }
  groups.push({ start: segmentStart, len: daysCount - segmentStart })
  return groups
}

/** Header kiểu Excel WBS: tháng → dải tuần (M/d–M/d) → số ngày → S M T W T F S; cuối tuần tô nền. */
...
export const GanttSidebarItem: FC<GanttSidebarItemProps> = ({ feature, onSelectItem, className }) => {
  ...
  return (
    <button
      className={cn('relative flex w-full cursor-pointer items-center gap-2.5 border-0 bg-transparent p-2.5 text-left text-xs hover:bg-secondary', className)}
      key={feature.id}
      onClick={handleClick}
    ...
    >
      ...
      />
      <p className="pointer-events-none flex-1 truncate font-medium">{feature.name}</p>
...
export const GanttSidebarHeader: FC = () => {
  const { labels } = useContext(GanttContext)
  return (
    <div
    ...
    >
      <p className="flex-1 truncate text-left">{labels.sidebarTaskColumn}</p>
      <p className="shrink-0">{labels.sidebarDuration}</p>
    </div>
  )
}
...
export const GanttFeatureDragHelper: FC<GanttFeatureDragHelperProps> = ({ direction, featureId, date }) => {
  ...
  return (
    <div
      className={cn('group -translate-y-1/2 !cursor-col-resize absolute top-1/2 z-[3] h-full w-6 rounded-md outline-none', direction === 'left' ? '-left-2.5' : '-right-2.5')}
      ref={setNodeRef}
      {...attributes}
      {...listeners}
    >
    ...
    >
      <ContextMenu>
        <ContextMenuTrigger asChild>
          ...
          >
            {label}
            <span className="max-h-[0] overflow-hidden opacity-80 transition-all group-hover:max-h-[2rem]">{formatDate(date, 'MMM dd, yyyy')}</span>
          </div>
        </ContextMenuTrigger>
        <ContextMenuContent>
          {onRemove ? (
...
export type GanttProviderProps = {
  ...
  children: ReactNode
  className?: string
  labels?: Partial<GanttUiLabels>
  /** Nếu có, timeline khởi tạo gồm mọi năm trong [min, max] (EVM: bọc toàn bộ ngày dự án + AC). */
...
export const GanttProvider: FC<GanttProviderProps> = ({
  ...
  const [timelineData, setTimelineData] = useState<TimelineData>(() => {
    if (initialYearRange != null && initialYearRange.min <= initialYearRange.max) {
      return buildTimelineFromYearRange(initialYearRange.min, initialYearRange.max)
    }
    return createInitialTimelineData(new Date())
  })
  const [, setScrollX] = useGanttScrollX()
  const [sidebarWidth, setSidebarWidth] = useState(0)
  const scrollAdjustRef = useRef<{ mode: 'prepend'; prevScrollWidth: number; prevScrollLeft: number } | null>(null)
  const isExtendingTimelineRef = useRef(false)
  const didInitialScrollCenterRef = useRef(false)

  const headerHeight = range === 'daily' ? 76 : 60
  const rowHeight = 36
  let columnWidth = 50
  ...
  useEffect(() => {
    if (initialYearRange == null || initialYearRange.min > initialYearRange.max) return
    setTimelineData(buildTimelineFromYearRange(initialYearRange.min, initialYearRange.max))
    didInitialScrollCenterRef.current = false
  }, [initialYearRange?.min, initialYearRange?.max])

  /** One-time center after layout can measure scrollWidth (sidebar width may update next). */
  useLayoutEffect(() => {
    const el = scrollRef.current
    if (!el || didInitialScrollCenterRef.current || el.scrollWidth <= el.clientWidth) return
    el.scrollLeft = el.scrollWidth / 2 - el.clientWidth / 2
    setScrollX(el.scrollLeft)
    didInitialScrollCenterRef.current = true
  }, [setScrollX, sidebarWidth, timelineData])

  /** After prepending a year, shift scrollLeft by the new content width so the view does not jump. */
  useLayoutEffect(() => {
    const anchor = scrollAdjustRef.current
    const el = scrollRef.current
    if (anchor?.mode !== 'prepend' || !el) {
      return
    }
    const delta = el.scrollWidth - anchor.prevScrollWidth
    if (delta > 0) {
      el.scrollLeft = Math.max(0, anchor.prevScrollLeft + delta)
      setScrollX(el.scrollLeft)
    }
  ...
  const handleScroll = useCallback(
    throttle(() => {
      ...
      if (isExtendingTimelineRef.current) {
        return
      }

      // Subpixel / elastic scroll can report ~0
      if (scrollLeft <= 1) {
        scrollAdjustRef.current = {
          mode: 'prepend',
          prevScrollWidth: scrollElement.scrollWidth,
        ...
        setTimelineData(prev => {
          ...
          return [buildYearBlock(firstYear - 1), ...prev]
        })
        return
      }
      ...
      if (scrollLeft + clientWidth >= scrollWidth - 1) {
        ...
        setTimelineData(prev => {
          ...
          return [...prev, buildYearBlock(lastYear + 1)]
        })
  ...
  const scrollToFeature = useCallback(
    ...
    [timelineData, zoom, range, columnWidth, sidebarWidth, onAddItem, labels]
  )
...
export const GanttToday: FC<GanttTodayProps> = ({ className }) => {
  const gantt = useContext(GanttContext)
  const label = gantt.labels.today
