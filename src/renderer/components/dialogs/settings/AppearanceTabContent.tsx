...
import { ShellTabOrderSettings } from '@/components/dialogs/settings/ShellTabOrderSettings'
...
export const AppearanceTabContent = memo(function AppearanceTabContent() {
  ...
  return (
    <>
      ...
      <Card id="settings-shell-tabs-card" className="gap-1.5 py-3 mb-4 rounded-md">
        <CardHeader className="gap-1 px-4 pb-0">
          <CardTitle className="flex flex-row items-center gap-1.5 text-sm">
            <PanelsTopLeft className="h-4 w-4" />
            {t('settings.shellTabs.title', 'Title Bar Tabs')}
          </CardTitle>
          <CardDescription className="text-xs">{t('settings.shellTabs.description', 'Choose which tabs appear on the title bar and drag to reorder them.')}</CardDescription>
        </CardHeader>
        <CardContent className="px-4 pt-2">
          <ShellTabOrderSettings />
        </CardContent>
      </Card>
    </>
  )
