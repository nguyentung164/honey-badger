...
const maxLength = 500
const defaultPageContentClass = 'ContentEditable__root relative block h-[calc(100vh-90px)] min-h-72 overflow-auto px-8 py-4 focus:outline-none'
const compactContentClass = 'ContentEditable__root relative block min-h-[12rem] max-h-[min(40vh,22rem)] min-w-0 flex-1 overflow-auto px-3 py-3 text-sm focus:outline-none'
...
export function Plugins({
  ...
  const resolvedContentClass = contentClassName ?? (compact ? compactContentClass : defaultPageContentClass)
    ...
    <div className="relative flex min-h-0 min-w-0 flex-1 flex-col">
      <ToolbarPlugin>
        {({ blockType }) => (
          <div
            className={cn(
              'vertical-align-middle sticky top-0 z-20 flex min-w-0 shrink-0 flex-wrap items-center gap-x-2 gap-y-1 border-b bg-background p-1',
        ...
        <RichTextPlugin
          contentEditable={
            <div className="min-h-0 min-w-0">
              <div className="min-h-0 min-w-0" ref={onRef}>
                <ContentEditable id={contentEditableId} placeholder={placeholder} className={resolvedContentClass} />
              </div>
            </div>
          }
