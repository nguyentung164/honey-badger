    ...
    <>
      <ToggleGroup type="single" value={elementFormat} defaultValue={elementFormat} onValueChange={handleValueChange}>
        ...
        {Object.entries(ELEMENT_FORMAT_OPTIONS).map(([value, option]) => (
          <ToggleGroupItem key={value} value={value} variant={'outline'} size="md" aria-label={option.name}>
            {option.icon}
          </ToggleGroupItem>
        ))}
      </ToggleGroup>
      ...
      {/* Indentation toggles */}
      <ToggleGroup type="single" value={elementFormat} defaultValue={elementFormat} onValueChange={handleValueChange}>
        <ToggleGroupItem value="outdent" aria-label="Outdent" variant={'outline'} size="md">
          <IndentDecreaseIcon className="size-4" />
        </ToggleGroupItem>

        <ToggleGroupItem value="indent" variant={'outline'} aria-label="Indent" size="md">
