...
import { matchSlashCommandTrigger } from '@/components/editor/plugins/slash-command-trigger'
...
export function MentionsPlugin(): JSX.Element | null {
  ...
  const options = useMemo(
    () => results.map(result => new MentionTypeaheadOption(result, <CircleUserRoundIcon className="size-4" />)).slice(0, SUGGESTION_LIST_LENGTH_LIMIT),
    [results]
  )
  ...
  const checkForMentionMatch = useCallback(
    (text: string) => {
      if (matchSlashCommandTrigger(text, editor) !== null) {
        return null
      }
    ...
    <LexicalTypeaheadMenuPlugin
      ...
      menuRenderFn={(
        ...
        return anchorElementRef.current && results.length
          ? createPortal(
              <div className="fixed z-[100] w-[200px] rounded-md border bg-popover shadow-md">
                <Command
                  onKeyDown={e => {
                    if (e.key === 'ArrowUp') {
                      e.preventDefault()
