...
import { exec, query } from '../schema/db'
...
export async function isBooleanColumn(tableName: string, columnName: string, fallback = true): Promise<boolean> {
  ...
  try {
    ...
    booleanColumnTypeCache.set(key, isBool)
    return isBool
  } catch {
    booleanColumnTypeCache.set(key, fallback)
    return fallback
  }
}
...
export async function listPrRepos(userId: string, projectId: string): Promise<PrRepo[]> {
  const rows = await query<any>('SELECT * FROM pr_repos WHERE user_id = ? AND project_id = ? ORDER BY name', [userId, projectId])
...
export async function upsertPrRepo(input: {
  ...
  const existing = await query<any>('SELECT id FROM pr_repos WHERE user_id = ? AND project_id = ? AND owner = ? AND repo = ? LIMIT 1', [
    input.userId,
    input.projectId,
    input.owner,
    input.repo,
  ...
  if (existing?.[0]) {
    ...
    await query(
      `UPDATE pr_repos SET name=?, local_path=?, remote_url=?, hosting=?, default_base_branch=?
       WHERE id=?`,
      [input.name, input.localPath ?? null, input.remoteUrl, input.hosting ?? 'github', input.defaultBaseBranch ?? 'stage', id]
    )
...
export async function listCheckpointTemplates(userId: string, projectId: string): Promise<PrCheckpointTemplate[]> {
  const rows = await query<any>('SELECT * FROM pr_checkpoint_templates WHERE user_id = ? AND project_id = ? ORDER BY sort_order, created_at', [userId, projectId])
...
export async function upsertCheckpointTemplate(input: {
  ...
  const isActiveBoolean = await isBooleanColumn('pr_checkpoint_templates', 'is_active')
  const isActiveValue = toDbBoolValue(input.isActive ?? true, isActiveBoolean)
  const existing = await query<any>('SELECT id FROM pr_checkpoint_templates WHERE user_id = ? AND project_id = ? AND code = ? LIMIT 1', [input.userId, input.projectId, input.code])
  if (existing?.[0]) {
    const id = existing[0].id
    if (input.headerGroupId !== undefined) {
      await query(`UPDATE pr_checkpoint_templates SET label=?, target_branch=?, sort_order=?, is_active=?, header_group_id=? WHERE id=?`, [
        input.label,
      ...
      await query(`UPDATE pr_checkpoint_templates SET label=?, target_branch=?, sort_order=?, is_active=? WHERE id=?`, [
        input.label,
        input.targetBranch ?? null,
        input.sortOrder ?? 0,
        isActiveValue,
...
export async function reorderCheckpointTemplates(userId: string, projectId: string, orderedIds: string[]): Promise<void> {
  for (let i = 0; i < orderedIds.length; i++) {
    await query('UPDATE pr_checkpoint_templates SET sort_order = ? WHERE id = ? AND project_id = ? AND user_id = ?', [i, orderedIds[i], projectId, userId])
  }
}
...
export async function listTrackedBranches(userId: string, projectId: string): Promise<TrackedBranchWithDetails[]> {
  ...
  const cpRows = await query<any>(
    `SELECT * FROM pr_branch_checkpoints
     WHERE tracked_branch_id IN (${placeholders})
     ORDER BY updated_at DESC, id DESC`,
    ids
  )
  const cpMap = new Map<string, Map<string, PrBranchCheckpoint>>()
  for (const cp of cpRows ?? []) {
    const mapped = mapCheckpoint(cp)
    const byTemplate = cpMap.get(mapped.trackedBranchId) ?? new Map<string, PrBranchCheckpoint>()
    // Nếu DB từng có record trùng (tracked_branch_id + template_id), giữ record mới nhất theo ORDER BY.
    if (!byTemplate.has(mapped.templateId)) {
      byTemplate.set(mapped.templateId, mapped)
    }
    cpMap.set(mapped.trackedBranchId, byTemplate)
  }
  return branchRows.map(r => ({
    ...
    checkpoints: [...(cpMap.get(r.id)?.values() ?? [])],
  }))
}
...
async function resolveUserIdForRepo(repoId: string): Promise<string | null> {
  const rows = await query<{ user_id: string }>('SELECT user_id FROM pr_repos WHERE id = ? LIMIT 1', [repoId])
...
export async function upsertTrackedBranch(input: {
  ...
  const id = input.id ?? randomUuidV7()
  await query(
    `INSERT INTO pr_tracked_branches (id, user_id, project_id, repo_id, branch_name, note, version)
     VALUES (?, ?, ?, ?, ?, ?, ?)`,
    [id, uid, input.projectId, input.repoId, input.branchName, input.note ?? null, 1]
  )
...
export async function deleteTrackedBranch(id: string): Promise<void> {
  // Tương thích DB cũ chưa có ON DELETE CASCADE cho fk_pr_bc_branch.
  await query('DELETE FROM pr_branch_checkpoints WHERE tracked_branch_id = ?', [id])
...
export async function deleteTrackedBranchesByIds(ids: string[]): Promise<number> {
  ...
  for (let i = 0; i < ids.length; i += TRACKED_DELETE_IDS_CHUNK) {
    ...
    await query(`DELETE FROM pr_branch_checkpoints WHERE tracked_branch_id IN (${ph})`, chunk)
    const res = await exec(`DELETE FROM pr_tracked_branches WHERE id IN (${ph})`, chunk)
...
async function resolveUserIdFromTrackedBranch(trackedBranchId: string): Promise<string | null> {
  const rows = await query<{ user_id: string }>('SELECT user_id FROM pr_tracked_branches WHERE id = ? LIMIT 1', [trackedBranchId])
  return rows?.[0]?.user_id ?? null
}
...
export async function upsertBranchCheckpoint(input: {
  ...
  const ghPrDraftBoolean = await isBooleanColumn('pr_branch_checkpoints', 'gh_pr_draft')
  const ghPrMergedBoolean = await isBooleanColumn('pr_branch_checkpoints', 'gh_pr_merged')
  const isDoneCast = isDoneBoolean ? 'boolean' : 'smallint'
  const ghPrDraftCast = ghPrDraftBoolean ? 'boolean' : 'smallint'
  const ghPrMergedCast = ghPrMergedBoolean ? 'boolean' : 'smallint'
  ...
  const existing = await query<any>(
    `SELECT id
     ...
     WHERE tracked_branch_id = ? AND template_id = ?
     ORDER BY updated_at DESC, id DESC`,
  ...
  if (existing?.[0]) {
    ...
    if (existing.length > 1) {
      ...
      if (staleIds.length > 0) {
        const stalePh = staleIds.map(() => '?').join(',')
  ...
  const ghDraftIns = 'ghPrDraft' in input ? (input.ghPrDraft == null ? null : toDbBoolValue(Boolean(input.ghPrDraft), ghPrDraftBoolean)) : null
  const ghMergedIns = 'ghPrMerged' in input ? (input.ghPrMerged == null ? null : toDbBoolValue(Boolean(input.ghPrMerged), ghPrMergedBoolean)) : null
  const isDoneIns = toDbBoolValue(input.isDone ?? false, isDoneBoolean)
  await query(
...
export async function listCheckpointKeysForRepoPr(owner: string, repo: string, prNumber: number): Promise<Array<{ trackedBranchId: string; templateId: string }>> {
  const o = owner.trim()
  const r = repo.trim()
...
export async function listPendingCheckpoints(): Promise<Array<PrBranchCheckpoint & { repoId: string; owner: string; repo: string; branchName: string; projectId: string }>> {
  ...
  const rows = await query<any>(
    `SELECT bc.*, b.repo_id, b.branch_name, b.project_id, r.owner, r.repo
     ...
     JOIN pr_repos r ON r.id = b.repo_id
     WHERE ${pendingWhere} AND bc.pr_number IS NOT NULL`
  )
  return (rows ?? []).map(r => ({
    ...mapCheckpoint(r),
    repoId: r.repo_id,
...
export async function setAutomationActive(id: string, isActive: boolean): Promise<void> {
  const isActiveBoolean = await isBooleanColumn('pr_automations', 'is_active')
  await query('UPDATE pr_automations SET is_active = ? WHERE id = ?', [toDbBoolValue(isActive, isActiveBoolean), id])
}
...
export async function getPrBoardSkippedBranchPatterns(userId: string, projectId: string): Promise<string[]> {
  const rows = await query<{ patterns_text: string | null }>(`SELECT patterns_text FROM pr_user_board_skip_branches WHERE user_id = ? AND project_id = ? LIMIT 1`, [
    userId,
    projectId,
  ])
...
export async function upsertPrBoardSkippedBranchPatterns(userId: string, projectId: string, lines: readonly string[]): Promise<void> {
  ...
  await query(
    `INSERT INTO pr_user_board_skip_branches (user_id, project_id, patterns_text) VALUES (?, ?, ?)
     ON CONFLICT (user_id, project_id) DO UPDATE SET patterns_text = EXCLUDED.patterns_text`,
...
export async function getPrAiAssistChatLines(userId: string, projectId: string): Promise<PrAiAssistChatLineJson[]> {
  const rows = await query<{ messages_json: string | null }>(`SELECT messages_json FROM pr_ai_assist_chats WHERE user_id = ? AND project_id = ? LIMIT 1`, [userId, projectId])
...
export async function upsertPrAiAssistChatLines(userId: string, projectId: string, lines: PrAiAssistChatLineJson[]): Promise<void> {
  ...
  await query(
    `INSERT INTO pr_ai_assist_chats (user_id, project_id, messages_json) VALUES (?, ?, ?)
     ON CONFLICT (user_id, project_id) DO UPDATE SET messages_json = EXCLUDED.messages_json`,
